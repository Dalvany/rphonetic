...
impl DoubleMetaphone {
    ...
    pub fn encode_alternate(&self, value: &str) -> Result<String, Infallible> {
        Ok(self.double_metaphone(value)?.alternate)
    }
    ...
    pub fn is_double_metaphone_equal(
        &self,
        value1: &str,
        value2: &str,
        alternate: bool,
        ...
        if alternate {
            Ok(result1?.alternate == result2?.alternate)
        } else {
            Ok(result1?.primary == result2?.primary)
        }
    }
    ...
    pub fn double_metaphone(&self, value: &str) -> Result<DoubleMetaphoneResult, Infallible> {
    ...
    fn condition_c0(value: &str, index: isize) -> bool {
        ...
        if index < 2
            || Self::char_at(value, index - 2)
                .is_some_and(|ch| is_vowel(Some(ch).map(|c| c.to_ascii_lowercase()), true))
        {
            return false;
        }
...
impl Encoder for DoubleMetaphone {
    ...
    fn encode(&self, value: &str) -> Result<String, Self::Error> {
        Ok(self.double_metaphone(value)?.primary)
    }
}
...
mod tests {
    ...
    fn assert_double_metaphone(expected: &str, source: &str) {
        ...
        assert_eq!(encoder.encode(source), Ok(expected.to_string()));
    }

    fn assert_double_metaphone_alternate(expected: &str, source: &str) {
        let encoder = DoubleMetaphone::default();
        assert_eq!(encoder.encode_alternate(source), Ok(expected.to_string()));
    }
    ...
    fn double_metaphone_not_equal_test(alternate: bool) {
        ...
        assert_eq!(
            encoder.is_double_metaphone_equal("Brain", "Band", alternate),
            Ok(false)
        );
        assert_eq!(
            encoder.is_double_metaphone_equal("Band", "Brain", alternate),
            Ok(false)
        );
    }
    ...
    fn test_c_cedilla() {
        ...
        assert_eq!(encoder.is_encoded_equals("\u{00e7}", "S"), Ok(true))
    }
    ...
    fn test_codec184() {
        ...
        assert_eq!(encoder.is_double_metaphone_equal("", "", false), Ok(true));
        assert_eq!(encoder.is_double_metaphone_equal("", "", true), Ok(true));
        assert_eq!(
            encoder.is_double_metaphone_equal("aa", "", false),
            Ok(false)
        );
        assert_eq!(encoder.is_double_metaphone_equal("aa", "", true), Ok(false));
        assert_eq!(
            encoder.is_double_metaphone_equal("", "aa", false),
            Ok(false)
        );
    ...
    fn test_is_double_metaphone_equal_extended3() {
        ...
        for (i, (data1, data2)) in FIXTURE.into_iter().enumerate() {
            let match1 = encoder
                .is_double_metaphone_equal(data1, data2, false)
                .unwrap();
            let match2 = encoder
                .is_double_metaphone_equal(data1, data2, true)
                .unwrap();
            if !match1 && !match2 {
    ...
    fn test_n_tilde() {
        ...
        assert_eq!(encoder.is_encoded_equals("\u{00f1}", "N"), Ok(true));
    }
    ...
    fn test_set_max_code_length() {
        ...
        assert_eq!(encoder.encode(value), Ok("JMPT".to_string()));
        assert_eq!(encoder.encode_alternate(value), Ok("AMPT".to_string()));

        let encoder = DoubleMetaphone::new(Some(3));
        assert_eq!(encoder.max_code_length, Some(3));
        assert_eq!(encoder.encode(value), Ok("JMP".to_string()));
        assert_eq!(encoder.encode_alternate(value), Ok("AMP".to_string()));
    }
    ...
    fn test_overflow_contains_case_1() {
        ...
        assert_eq!(result, Ok("LN".to_string()));
    }
    ...
    fn test_overflow_contains_case_2() {
        ...
        assert_eq!(result, Ok("XL".to_string()));
    }
    ...
    fn test_unbounded_2() {
        ...
        assert_eq!(result, Ok("SNKRNSXN".to_string()));
    }
}
